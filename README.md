...
相对于传统的`Key-Value`结构，解决*A**n**n*问题的缓存需要进行特殊设计，对于一个优惠集合而言，*A**n*3意味着需要缓存*n*×(*n*−1)×(*n*−2)条数据。当*n*=7时，需要缓存210条数据。为了在内存大小和缓存带来的性能收益之间取得平衡，*A**n*3是最合适的级别。
...
从稳定性角度考虑，我们需要对计算次数进行统计。在压力测试中，我们通过`LimitingUtil.count`方法来统计进入`calc`方法的次数。显然，在没有开启缓存的情况下，计算次数为*A**n**n*×*n*，而当开启缓存时，计算次数为*A**n**n*×(*n*−3)+*A**n*3。
